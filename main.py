...
def main():
    ...
    # Generate pallets list based on the user's input
    ...
    temporary_location: Location = Location('Temporary Location 1', 250)

    overflow1: Location = Location('Overflow1', 50)

    overflow2: Location = Location('Overflow2', 50)

    over_pallets = overflow1.load_pallet(pallets=loaded_pallets, max_allowed=50)
    remaining_pallets = generated_pallets[loading_input:]

    overflow1.relocate_pallet(next_location=overflow2)

    temporary_location.load_pallet(pallets=over_pallets, max_allowed=250)



    print(f"Temporary Location Pallets: {len(temporary_location.pallets)}")
    print(f"Overflow1 Pallets: {len(overflow1.pallets)}")
    print(f"Unloaded Pallets: {len(remaining_pallets)}")
