...
class SystemMenu(Pallet):
    ...
    def _show_instructions(self):
        ...
        print('4 - Show locations')
        print('5 - Show location status')  # How many pallets we have and where they are, ask which location
        print('6 - Exit')
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if user_input == '2':
                ...
                load_pallets = location.load_pallet(load_pallets, location.limit)

                choose_location = input('Where to load?: ')
                if choose_location in locations_data:
                    location = locations_data[choose_location]
                    load_pallet = location.load_pallet(gen_pallet, location.limit)
                    print(f"Loading {num_pallets_to_load} pallets to {choose_location}")
                    print(f"{len(load_pallet)} pallets could not be loaded")
                    print(f'{[p for p in gen_pallet]}')
                else:
