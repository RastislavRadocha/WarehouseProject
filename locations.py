class Location:
    ...
    def offload_pallet(self, num_pallets):
        self.limit += num_pallets
        print(f'{num_pallets} pallets off-loaded from {self.name}')
        if self.limit == num_pallets:
            print(f'{self.limit} empty locations available on {self.name}')
